...
// ═══════════════════════════════════════════════════════════════════════════════
// FUNDING ARBITRAGE — Cross-exchange scoring engine
// ═══════════════════════════════════════════════════════════════════════════════

const ARB_REFRESH_MS = 30_000;
let _arbFilter   = 'all';
let _arbAllOpps  = [];

function setArbFilter(filter, btn) {
  _arbFilter = filter;
  document.querySelectorAll('[data-arb]').forEach(b => b.classList.remove('active'));
  if (btn) btn.classList.add('active');
  renderArbGrid(_arbAllOpps);
}

async function fetchArbOpportunities() {
  const minApr = Number(document.getElementById('arbMinApr')?.value ?? 20);
  try {
    const res = await fetch(`/api/funding-arb/opportunities?min_spread=${minApr}&top_n=50`);
    if (!res.ok) return;
    const data = await res.json();
    _arbAllOpps = data.opportunities || [];
    renderArbSection(data);
  } catch (e) {
    console.warn('[ARB] fetch error:', e);
  }
}

function renderArbSection(data) {
  const opps  = data.opportunities || [];
  const stats = data.stats || {};

  // Badge
  const badge = document.getElementById('arbScanBadge');
  if (badge) {
    if (opps.length > 0) {
      badge.textContent = `${opps.length} OPORTUNIDADES`;
      badge.style.background = 'var(--green)';
      badge.style.color = '#000';
    } else {
      badge.textContent = 'SEM DADOS';
      badge.style.background = 'var(--border-md)';
      badge.style.color = 'var(--text-muted)';
    }
  }

  // Meta bar
  const metaBar = document.getElementById('arbMetaBar');
  if (metaBar && stats.total_opportunities > 0) {
    const bestAprStr = stats.best_apr ? stats.best_apr.toFixed(0) + '%/yr' : '';
    metaBar.innerHTML =
      `<span style="color:var(--text-muted);font-size:11px">` +
      `<b style="color:var(--text)">${stats.total_opportunities}</b> oportunidades` +
      ` &nbsp;·&nbsp; <b style="color:var(--red)">${stats.hot_count || 0}</b> HOT` +
      ` &nbsp;·&nbsp; Melhor: <b style="color:var(--green)">${stats.best_symbol || '—'}</b>` +
      (bestAprStr ? ` <span style="color:var(--green)">${bestAprStr}</span>` : '') +
      ` &nbsp;·&nbsp; Média APR: <b style="color:var(--text)">${stats.avg_apr ? stats.avg_apr.toFixed(0) + '%' : '—'}</b>` +
      ` &nbsp;·&nbsp; Confiança: <b style="color:var(--yellow)">${stats.avg_confidence ? stats.avg_confidence.toFixed(0) : '—'}</b>/100` +
      `</span>`;
  }
...
function renderArbGrid(opps) {
  ...
  if (_arbFilter !== 'all') {
    filtered = opps.filter(o =>
      o.tier === _arbFilter || o.arb_type === _arbFilter
    );
  }

  if (!filtered.length) {
    grid.innerHTML = '<div class="arb-empty">Sem oportunidades com os filtros selecionados</div>';
    return;
  }

  grid.innerHTML = filtered.map(o => {
    const tierClass = o.tier === 'HOT' ? 'tier-hot' :
                      o.tier === 'BLUE_CHIP' ? 'tier-blue' : '';

    const tierBadge = o.tier === 'HOT'
      ? '<span class="arb-tier-badge arb-tier-hot">HOT 🔥</span>'
      : o.tier === 'BLUE_CHIP'
      ? '<span class="arb-tier-badge arb-tier-blue">BLUE CHIP</span>'
      : '<span class="arb-tier-badge arb-tier-norm">NORMAL</span>';

    const aprColor = o.is_profitable ? 'var(--green)' : 'var(--text-dim)';

    const longFr  = (o.long_funding_8h  >= 0 ? '+' : '') + o.long_funding_8h.toFixed(4)  + '%';
    const shortFr = (o.short_funding_8h >= 0 ? '+' : '') + o.short_funding_8h.toFixed(4) + '%';
    ...
    const volStr = fmtUsd(o.vol_usd_24h);
    const oiStr  = o.oi_usd > 0 ? fmtUsd(o.oi_usd) : '—';

    const typeLabel = o.arb_type === 'cross_exchange' ? 'CROSS'
                    : o.arb_type === 'cash_carry'     ? 'CARRY'
                    : 'REVERSE';

    const pnlDaily   = o.is_profitable ? '$' + o.pnl_daily_10k.toFixed(2)   : '—';
    const pnlWeekly  = o.is_profitable ? '$' + o.pnl_weekly_10k.toFixed(2)  : '—';
    const pnlMonthly = o.is_profitable ? '$' + o.pnl_monthly_10k.toFixed(2) : '—';

    const beColor = o.breakeven_cycles < 3 ? 'var(--green)' : 'var(--yellow)';
    const beStr   = o.breakeven_cycles < 100 ? o.breakeven_cycles + ' ciclos' : '∞';

    const nsColor = o.net_spread_8h_pct > 0 ? 'var(--green)' : 'var(--bear)';

    const symBase  = o.symbol.replace('USDT', '');
    const longFrColor  = o.long_funding_8h  < 0 ? 'var(--bull)' : 'var(--text-muted)';
    const shortFrColor = o.short_funding_8h > 0 ? 'var(--bear)' : 'var(--text-muted)';

    return `
    <div class="arb-card ${tierClass} ${o.is_profitable ? '' : 'arb-not-profitable'}">
      <div class="arb-card-header">
        <span class="arb-symbol">${symBase}<span style="color:var(--text-muted);font-size:10px">/USDT</span></span>
        ${tierBadge}
      </div>
      <div class="arb-exchanges">
        <span class="arb-ex-long">L ${o.long_exchange.toUpperCase()}</span>
        <span style="color:var(--text-dim)">→</span>
        <span class="arb-ex-short">S ${o.short_exchange.toUpperCase()}</span>
        <span style="margin-left:auto;font-size:9px;color:var(--text-dim)">${typeLabel}</span>
      </div>
      <div class="arb-apr-row">
        <span class="arb-apr-value" style="color:${aprColor}">
          ${o.net_apr.toFixed(0)}<span style="font-size:11px;font-weight:400;color:var(--text-muted)">%/yr</span>
        </span>
        <span class="arb-apr-label">Score: <b style="color:var(--yellow)">${o.confidence}</b>/100</span>
      </div>
      <div class="arb-confidence-bar">
        <div class="arb-confidence-fill" style="width:${o.confidence}%"></div>
      </div>
      <div class="arb-metrics">
        <span class="arb-metric-label">Long FR</span>
        <span class="arb-metric-value" style="color:${longFrColor}">${longFr}/8h</span>
        <span class="arb-metric-label">Short FR</span>
        <span class="arb-metric-value" style="color:${shortFrColor}">${shortFr}/8h</span>
        <span class="arb-metric-label">Vol 24h</span>
        <span class="arb-metric-value">${volStr}</span>
        <span class="arb-metric-label">OI</span>
        <span class="arb-metric-value">${oiStr}</span>
        <span class="arb-metric-label">Breakeven</span>
        <span class="arb-metric-value" style="color:${beColor}">${beStr}</span>
        <span class="arb-metric-label">Net Spread</span>
        <span class="arb-metric-value" style="color:${nsColor}">${o.net_spread_8h_pct.toFixed(4)}%</span>
      </div>
      <div class="arb-pnl-row">
        <div class="arb-pnl-item">
          <span class="arb-pnl-val">${pnlDaily}</span>
          <span class="arb-pnl-lbl">dia/10K</span>
        </div>
        <div class="arb-pnl-item">
          <span class="arb-pnl-val">${pnlWeekly}</span>
          <span class="arb-pnl-lbl">sem/10K</span>
        </div>
        <div class="arb-pnl-item">
          <span class="arb-pnl-val">${pnlMonthly}</span>
          <span class="arb-pnl-lbl">mês/10K</span>
        </div>
      </div>
    </div>`;
  }).join('');
...
setInterval(fetchArbOpportunities, ARB_REFRESH_MS);
