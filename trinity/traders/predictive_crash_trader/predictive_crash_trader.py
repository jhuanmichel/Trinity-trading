...
def _send_crash_telegram(c: dict):
    try:
        ...
        try:
            ...
            _active_thr = get_active_threshold("crash", is_blue_chip=_is_bc, fallback=ALERT_THRESHOLD)
            if opp_score < _active_thr:
                log.debug(f"[AUTO_THRESHOLD] {symbol} crash score {opp_score:.1f} < {_active_thr}")
                return
            try:
                from trinity.traders.btc_regime_monitor import get_btc_regime as _gbr
                ...
                _reg = "UNKNOWN"
            _passes, _reason = passes_auto_learning_filters(symbol, opp_score, "SHORT", _reg)
            if not _passes:
                log.info(f"[AUTO_FILTER] {symbol} crash bloqueado: {_reason}")
                return
