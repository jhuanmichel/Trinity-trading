...
def _cleanup_caches() -> None:
    ...
    if stale_ls or stale_fund or stale_oi:
        log.debug(
            f"[CrashScanner] Cache cleanup: -{len(stale_ls)} ls, "
...
def _fetch_universe() -> List[dict]:
    ...
    try:
        ...
        if not isinstance(tickers_raw, list):
            log.error(f"[CrashScanner] Formato inesperado: {type(tickers_raw)}")
            return []
        ...
        log.error(f"[CrashScanner] Erro ao buscar universo: {e}")
        return []
