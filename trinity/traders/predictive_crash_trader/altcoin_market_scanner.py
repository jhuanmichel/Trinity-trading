"""
altcoin_market_scanner.py — Scanner Gate.io Futures (Cap. 19)
...
Campos Gate.io:
  ...
  - last:             preço string
  - high_24h / low_24h
  - volume_24h_quote: volume 24h em USDT
  - change_percentage: variação % (já em %, não decimal)
  - funding_rate:     funding atual string
  - total_size:       OI em contratos
"""
import logging
import os
import sys
...
def scan_universe() -> List[dict]:
    ...
    log.info("[Scanner] Atualizando universo Gate.io Futures...")
    raw = _fetch_universe()
    if not raw:
...
def _fetch_universe() -> List[dict]:
    try:
        ...
        if not isinstance(tickers, list):
            log.error(f"[Scanner] Formato inesperado Gate.io: {type(tickers)}")
            return []
        return [
            t for t in tickers
        ...
        log.error(f"[Scanner] Erro ao buscar universo Gate.io: {e}")
        return []
...
def _get_ticker(gate_symbol: str) -> dict:
    try:
        ...
        log.debug(f"[Scanner] Ticker error {gate_symbol}: {e}")
        return {}
